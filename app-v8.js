...
// ---------- TRIVIAL ----------
const trivialHomeScreen=document.getElementById("trivialHomeScreen");
const trivialGameScreen=document.getElementById("trivialGameScreen");
const trivialFinishScreen=document.getElementById("trivialFinishScreen");
let TRIVIAL_QUESTIONS=[];
let trivialGame=null;
let trivialLoaded=false;

async function loadTrivialQuestions(){
  if(trivialLoaded && TRIVIAL_QUESTIONS.length)return true;
  const msg=document.getElementById("trivialLoadMessage");
  if(msg)msg.textContent="Cargando preguntas…";
  try{
    const res=await fetch("trivial_questions.json",{cache:"no-store"});
    if(!res.ok)throw new Error(`HTTP ${res.status}`);
    const data=await res.json();
    if(!Array.isArray(data) || data.length<10)throw new Error("Banco de preguntas inválido");
    TRIVIAL_QUESTIONS=data;
    trivialLoaded=true;
    if(msg)msg.textContent=`${data.length} preguntas disponibles.`;
    renderTrivialCategories();
    return true;
  }catch(err){
    console.error(err);
    if(msg)msg.textContent="❌ No se pudo cargar el banco de preguntas. Comprueba que trivial_questions.json esté subido junto al resto de archivos.";
    return false;
  }
}

async function showTrivialHome(){
  hideAllScreens();
  trivialHomeScreen.classList.remove("hidden");
  await loadTrivialQuestions();
}

function renderTrivialCategories(){
  const box=document.getElementById("trivialCategories");
  box.innerHTML="";
  const cats=[...new Set(TRIVIAL_QUESTIONS.map(q=>q.category))].sort((a,b)=>a.localeCompare(b,"es"));
  const icons={"Arte":"🎨","Ciencia":"🔬","Cine":"🎬","Cultura general":"🧠","Deportes":"⚽","España":"🇪🇸","Geografía":"🌍","Historia":"🏛️","Lengua":"🔤","Literatura":"📚","Música":"🎵","Naturaleza":"🌿"};
  cats.forEach(cat=>{
    const count=TRIVIAL_QUESTIONS.filter(q=>q.category===cat).length;
    const b=document.createElement("button");
    b.type="button"; b.className="level trivial-category";
    b.innerHTML=`<span>${icons[cat]||"❓"}</span><b>${cat}</b><small>${count} preguntas</small>`;
    b.addEventListener("click",()=>startTrivial(cat));
    box.appendChild(b);
  });
}

function shuffleTrivia(arr){
  const a=[...arr];
  for(let i=a.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[a[i],a[j]]=[a[j],a[i]];}
  return a;
}

async function startTrivial(category=null){
  if(!(await loadTrivialQuestions()))return;
  const pool=category?TRIVIAL_QUESTIONS.filter(q=>q.category===category):TRIVIAL_QUESTIONS;
  if(pool.length<1)return;
  const rounds=Math.min(10,pool.length);
  trivialGame={category,questions:shuffleTrivia(pool).slice(0,rounds),index:0,score:0,answered:false};
  showTrivialQuestion();
}

function showTrivialQuestion(){
  if(!trivialGame)return;
  if(trivialGame.index>=trivialGame.questions.length){finishTrivial();return;}
  hideAllScreens(); trivialGameScreen.classList.remove("hidden");
  const q=trivialGame.questions[trivialGame.index];
  trivialGame.answered=false;
  document.getElementById("trivialGameTitle").textContent=`Pregunta ${trivialGame.index+1}/${trivialGame.questions.length}`;
  document.getElementById("trivialCategoryLabel").textContent=trivialGame.category||"Mixto";
  document.getElementById("trivialProgress").textContent=`${trivialGame.index+1}/${trivialGame.questions.length}`;
  document.getElementById("trivialScore").textContent=`✅ ${trivialGame.score}`;
  document.getElementById("trivialQuestionCategory").textContent=q.category;
  document.getElementById("trivialQuestionText").textContent=q.q;
  const answers=document.getElementById("trivialAnswers");
  answers.innerHTML="";
  const letters=["A","B","C","D"];
  q.options.forEach((opt,i)=>{
    const b=document.createElement("button"); b.type="button"; b.className="trivial-answer";
    b.innerHTML=`<span class="trivial-letter">${letters[i]}</span><span>${opt}</span>`;
    b.addEventListener("click",()=>answerTrivial(i)); answers.appendChild(b);
  });
  const fb=document.getElementById("trivialFeedback"); fb.classList.add("hidden"); fb.innerHTML="";
}

function answerTrivial(choice){
  if(!trivialGame || trivialGame.answered)return;
  trivialGame.answered=true;
  const q=trivialGame.questions[trivialGame.index];
  const buttons=[...document.querySelectorAll("#trivialAnswers .trivial-answer")];
  buttons.forEach((b,i)=>{b.disabled=true;if(i===q.answer)b.classList.add("correct");if(i===choice && i!==q.answer)b.classList.add("wrong");});
  const good=choice===q.answer;
  if(good){trivialGame.score++;haptic("success");}else haptic("error");
  document.getElementById("trivialScore").textContent=`✅ ${trivialGame.score}`;
  const fb=document.getElementById("trivialFeedback");
  fb.classList.remove("hidden");
  fb.innerHTML=`<strong>${good?"✅ ¡Correcto!":"❌ Incorrecto"}</strong><p>${good?"¡Punto para ese coco!":`La respuesta correcta era: ${q.options[q.answer]}.`}</p><button id="trivialNextBtn" class="primary wide">${trivialGame.index+1>=trivialGame.questions.length?"Ver resultado":"Siguiente pregunta ›"}</button>`;
  document.getElementById("trivialNextBtn").addEventListener("click",()=>{trivialGame.index++;showTrivialQuestion();});
}

function finishTrivial(){
  hideAllScreens(); trivialFinishScreen.classList.remove("hidden");
  const total=trivialGame.questions.length, score=trivialGame.score, pct=Math.round(score/total*100);
  let title="¡Buen entrenamiento! 🧠";
  if(pct===100)title="¡Pleno absoluto! 🏆"; else if(pct>=80)title="¡Coco enciclopédico! 🔥"; else if(pct>=60)title="¡Muy buena ronda! 👏";
  document.getElementById("trivialResultTitle").textContent=title;
  document.getElementById("trivialResultText").textContent=`Has acertado ${score} de ${total} preguntas.`;
  document.getElementById("trivialFinalScore").textContent=`${score}/${total}`;
  document.getElementById("trivialFinalPercent").textContent=`${pct}%`;
  document.getElementById("trivialFinalCategory").textContent=trivialGame.category||"Mixto";
}

document.getElementById("openTrivialBtn").addEventListener("click",showTrivialHome);
document.getElementById("trivialMenuBack").addEventListener("click",showMainMenu);
document.getElementById("trivialMixedBtn").addEventListener("click",()=>startTrivial(null));
document.getElementById("trivialBackBtn").addEventListener("click",showTrivialHome);
document.getElementById("trivialAgainBtn").addEventListener("click",()=>startTrivial(trivialGame?.category||null));
document.getElementById("trivialHomeBtn").addEventListener("click",showMainMenu);
